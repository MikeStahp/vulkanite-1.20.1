...
void main() {
    ...
    // Signed world-space unit normal in RGB. Alpha deliberately does not encode
    // a terrain face, so entity pixels cannot publish persistent block-face
    // blocklight cache entries.
    colortex3 = vec4(normalize(normal), 0.0);
