...
// ----------------------------------------------------------------------------
// Low-Discrepancy Sampling (R2 Sequence)
// ----------------------------------------------------------------------------

// Golden ratio and plastic constant for quasi-random sampling
const float GOLDEN_RATIO = 1.6180339887;
const float PLASTIC_CONST = 1.324717957;

/// R2 sequence - quasi-random with good 2D distribution
/// Better than pure random, zero texture cost
/// @param n Sample index
/// @return 2D quasi-random point in [0,1)^2
...
/// @param sample2D The 2D sample point to jitter
/// @param frameIndex Current frame index
/// @return Temporally jittered sample point
...
/// Interleaved gradient noise - cheap blue noise approximation
/// Good for screen-space sampling patterns
...
/// 2D temporal blue noise
/// @param coord Screen coordinate
/// @param frame Current frame index
/// @return 2D noise values in [0, 1)^2
...
/// Used for probabilistic path termination in path tracing
/// @param throughput The current path throughput (RGB)
/// @return Probability of continuing the path [0.05, 1.0]
float russianRouletteProb(vec3 throughput) {
    return clamp(max(throughput.r, max(throughput.g, throughput.b)), 0.05, 1.0);
}

// ----------------------------------------------------------------------------
// GGX Importance Sampling for Specular Reflections
// ----------------------------------------------------------------------------

/// GGX Importance Sampling for microfacet BRDF
/// Samples a microfacet normal from the GGX distribution
/// This produces physically correct glossy reflections for metals
/// @param xi Two random numbers in [0,1)
/// @param N The surface normal (must be normalized)
/// @param roughness Surface roughness [0.04, 1.0]
/// @param H Output: sampled half-vector (microfacet normal)
/// @return The PDF of the sampled direction
float importanceSampleGGX(vec2 xi, vec3 N, float roughness, out vec3 H) {
    float a = roughness * roughness;
    float a2 = a * a;
    
    // Spherical coordinates for GGX distribution
    float phi = 2.0 * PI * xi.x;
    float cosTheta = sqrt((1.0 - xi.y) / (1.0 + (a2 - 1.0) * xi.y));
    float sinTheta = sqrt(1.0 - cosTheta * cosTheta);
    
    // Convert from spherical to Cartesian in tangent space
    vec3 tangentH = vec3(
        sinTheta * cos(phi),
        sinTheta * sin(phi),
        cosTheta
    ...
    H = normalize(tangentH.x * T + tangentH.y * B + tangentH.z * N);
    
    // Compute PDF: D(h) * cos(theta_h) / (4 * dot(H, V))
    // For reflection: PDF = D(h) * cos(theta_h) / (4 * dot(H, V))
    // We return D(h) * cos(theta_h) for the caller to divide by 4*HdotV
    float D = a2 / (PI * (cosTheta * cosTheta * (a2 - 1.0) + 1.0) * (cosTheta * cosTheta * (a2 - 1.0) + 1.0) + 1e-5);
    return D * cosTheta;
}

/// Sample reflection direction using GGX importance sampling
/// Returns the sampled reflection direction and the PDF
/// @param V View direction (from surface to camera, normalized)
/// @param N Surface normal (normalized)
/// @param roughness Surface roughness
/// @param seed Random seed (modified in place)
/// @param pdf Output: PDF of the sampled direction
/// @return Sampled reflection direction (normalized)
vec3 sampleGGXReflection(vec3 V, vec3 N, float roughness, inout uint seed, out float pdf) {
    // Generate two random numbers
    vec2 xi = vec2(rand(seed), rand(seed));
    
    // Sample microfacet normal from GGX distribution
    vec3 H;
    float DdotN = importanceSampleGGX(xi, N, roughness, H);
    
    // Compute reflection direction
    vec3 L = reflect(-V, H);
    
    // Compute the PDF for the reflection direction
    // PDF = D(h) * cos(theta_h) / (4 * dot(H, V))
...
/// Evaluate GGX BRDF for the given reflection direction
/// Returns the BRDF value and computes the PDF
/// @param N Surface normal
/// @param V View direction (surface to camera)
/// @param L Light/reflection direction
/// @param F0 Fresnel reflectance at normal incidence
/// @param roughness Surface roughness
/// @param pdf Output: PDF of this direction
/// @return BRDF value (specular reflectance)
vec3 evaluateGGXBRDF(vec3 N, vec3 V, vec3 L, vec3 F0, float roughness, out float pdf) {
    ...
    float k = (a + 1.0) * (a + 1.0) / 8.0; // Disney remapping
    ...
    // BRDF value
    vec3 brdf = (D * G * F) / (4.0 * NdotV * NdotL + 1e-5);
    
    // PDF for importance sampling
    pdf = (D * NdotH) / (4.0 * HdotV + 1e-5);
