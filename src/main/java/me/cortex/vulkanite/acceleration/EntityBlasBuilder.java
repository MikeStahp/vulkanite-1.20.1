...
public class EntityBlasBuilder {
    ...
    private record CachedBuffer(VRef<VBuffer> buffer, int usage, int memoryProperties, int flags) {}
    private final List<CachedBuffer> reusableBuffers = new ArrayList<>();
    private final List<VRef<VAccelerationStructure>> reusableAS = new ArrayList<>();
    ...
    public void free() {
        for (var cached : reusableBuffers) {
            cached.buffer.close();
        }
        reusableBuffers.clear();
        for (var as : reusableAS) {
            as.close();
        }
        reusableAS.clear();
    }

    private VRef<VBuffer> getBuffer(long size, int usage, int memoryProperties, int alignment, int flags) {
        for (var cached : reusableBuffers) {
            if (cached.buffer.get().getRefCount() == 1 &&
                    cached.buffer.get().size() >= size &&
                    cached.usage == usage &&
        ...
        var buffer = ctx.memory.createBuffer(size, usage, memoryProperties, alignment, flags);
        reusableBuffers.add(new CachedBuffer(buffer.addRef(), usage, memoryProperties, flags));
        return buffer;
    }

    private VRef<VAccelerationStructure> getAS(long size) {
        for (var ref : reusableAS) {
            if (ref.get().getRefCount() == 1 && ref.get().size() >= size) {
                return ref.addRef();
            }
        }
        ...
        reusableAS.add(structure.addRef());
        return structure;
    }
    ...
    private VRef<VAccelerationStructure> executeBlasBuild(VContext ctx, VCmdBuff cmd, MemoryStack stack, VkAccelerationStructureGeometryKHR.Buffer geometryInfos, int[] prims) {
        ...
        var structure = getAS(buildSizesInfo.accelerationStructureSize());
