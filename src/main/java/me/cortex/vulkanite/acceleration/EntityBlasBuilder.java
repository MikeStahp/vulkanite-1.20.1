...
public class EntityBlasBuilder {
        ...
        private static VRef<VAccelerationStructure> executeBlasBuild(VContext ctx, VCmdBuff cmd, MemoryStack stack,
                        VkAccelerationStructureGeometryKHR.Buffer geometryInfos, int[] prims) {
                ...
                vkCmdPipelineBarrier(cmd.buffer(), VK_PIPELINE_STAGE_ACCELERATION_STRUCTURE_BUILD_BIT_KHR,
        ...
        List<BLASResult> buildBlas(List<Pair<RenderLayer, BufferBuilder.BuiltBuffer>> renders, VCmdBuff cmd) {
                ...
                for (var type : renders) {
                        // if (((RenderLayer.MultiPhase) type.getLeft()).phases.texture instanceof
                        // RenderPhase.Textures) {
                        // throw new IllegalStateException("Multi texture not supported");
                        // }
                        // var textureId = ((RenderLayer.MultiPhase)
                        // type.getLeft()).phases.texture.getId().get();
                        // var texture = textureManager.getTexture(textureId);
                        // var vkImage = ((IVGImage) texture).getVGImage();
                        // if (vkImage == null) {
                        // throw new IllegalStateException("Vulkan texture not created for render layer
                        // " + type.getLeft());
                        // }
                        ...
                        combined_size += type.getRight().getVertexBuffer().remaining() + 256;// Add just some buffer so
                                                                                             // we can do alignment etc
                }
                // Each render layer gets its own geometry entry in the blas

                // TODO: PUT THE BINDLESS TEXTURE REFERENCE AT THE START OF THE render layers
                // geometry buffer
                ...
                for (var pair : renders) {
                        ...
                        MemoryUtil.memCopy(MemoryUtil.memAddress(pair.getRight().getVertexBuffer()), ptr + offset,
                                        pair.getRight().getVertexBuffer().remaining());
                        infos.add(new BuildInfo(pair.getRight().getParameters().format(),
                                        pair.getRight().getParameters().indexCount() / 6,
                ...
                cmd.encodeBufferBarrier(geometryBuffer, 0, combined_size, VK_PIPELINE_STAGE_TRANSFER_BIT,
        ...
        private VkAccelerationStructureGeometryKHR.Buffer populateBuildStructs(VContext ctx, MemoryStack stack,
                        VCmdBuff cmdBuff, List<BuildInfo> geometries, int[] primitiveCounts) {
