...
public class EntityBlasBuilder {
    ...
    private final List<VRef<VBuffer>> reusableBuffers = new ArrayList<>();
    ...
    private VRef<VBuffer> getBuffer(long size, int usage, int properties, int vmaFlags, long alignment) {
        for (VRef<VBuffer> ref : reusableBuffers) {
            VBuffer buf = ref.get();
            if (buf.getRefCount() == 1 &&
                    buf.size() >= size &&
                    buf.usage() == usage &&
                    buf.memoryProperties() == properties &&
                    buf.vmaFlags() == vmaFlags) {
                return ref.addRef();
            }
        }
        var newBuf = ctx.memory.createBuffer(size, usage, properties, alignment, vmaFlags);
        reusableBuffers.add(newBuf);
        return newBuf.addRef();
    }

    public void free() {
        for (var ref : reusableBuffers) {
            ref.close();
        }
    ...
    private VRef<VAccelerationStructure> executeBlasBuild(VContext ctx, VCmdBuff cmd, MemoryStack stack, VkAccelerationStructureGeometryKHR.Buffer geometryInfos, int[] prims) {
        ...
        var backing = getBuffer(buildSizesInfo.accelerationStructureSize(),
        ...
        var structure = ctx.memory.createAcceleration(backing, 0, buildSizesInfo.accelerationStructureSize(), VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR);
        backing.close();
