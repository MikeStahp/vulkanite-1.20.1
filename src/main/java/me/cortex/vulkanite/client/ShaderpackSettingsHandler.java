...
public class ShaderpackSettingsHandler {
    ...
    public static void applyShaderpackSettings(IrisRenderingPipeline pipeline) {
        try {
            ...
            System.err
                    .println("[Vulkanite] Failed to access shaderpack settings through reflection: " + e.getMessage());
        }
    }
    ...
    private static void applySettingsFromShaderProperties(ShaderProperties shaderProperties) {
        ...
        try {
            ...
            if (variables.containsKey("enableRestir")) {
                me.cortex.vulkanite.client.config.DLSSConfig dlssConfig = me.cortex.vulkanite.client.config.DLSSConfig
                        .load();
                String value = variables.get("enableRestir");
                if ("true".equals(value)) {
                    dlssConfig.setReSTIREnabled(true);
                    dlssConfig.save();
                    System.out.println("[Vulkanite] Enabled ReSTIR from shaderpack settings");
                } else if ("false".equals(value)) {
                    dlssConfig.setReSTIREnabled(false);
                    dlssConfig.save();
            ...
            if (variables.containsKey("restirReservoirWidth")) {
                try {
                    ...
                    System.out.println(
                            "[Vulkanite] Set ReSTIR reservoir width to " + width + " from shaderpack settings");
            ...
            if (variables.containsKey("restirReservoirHeight")) {
                try {
                    ...
                    System.out.println(
                            "[Vulkanite] Set ReSTIR reservoir height to " + height + " from shaderpack settings");
                } catch (NumberFormatException e) {
