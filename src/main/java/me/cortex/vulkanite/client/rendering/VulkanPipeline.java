...
public class VulkanPipeline {
    ...
    public void renderPostShadows(List<VRef<VGImage>> vgOutImgs, Camera camera, ShaderStorageBuffer[] ssbos,
        ...
        // Use an explicit ArrayList allocation with pre-sized capacity instead of Stream.toList() to avoid per-frame GC allocations
        var outImgs = new ArrayList<VRef<VImage>>(outImgsSize);
        for (int i = 0; i < outImgsSize; i++) {
