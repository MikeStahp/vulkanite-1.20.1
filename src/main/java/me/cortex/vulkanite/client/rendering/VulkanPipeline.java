...
public class VulkanPipeline {
    ...
    public void renderPostShadows(List<VRef<VGImage>> vgOutImgs, Camera camera, ShaderStorageBuffer[] ssbos,
        ...
        // ⚡ Bolt: Replaced stream operations with standard loops to eliminate
        // O(N) per-frame iterator allocations and lambda overhead, reducing GC pressure
        int[] outImgsGlIds = new int[vgOutImgs.size()];
        int[] outImgsGlLayouts = new int[vgOutImgs.size()];
        int iIdx = 0;
        for (var img : vgOutImgs) {
            outImgsGlIds[iIdx] = img.get().glId;
            outImgsGlLayouts[iIdx] = GL_LAYOUT_GENERAL_EXT;
            iIdx++;
        }
        ...
        // ⚡ Bolt: Loop replacement avoids intermediate stream allocation
