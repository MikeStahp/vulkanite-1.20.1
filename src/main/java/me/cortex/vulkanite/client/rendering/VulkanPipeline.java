...
public class VulkanPipeline {
    ...
    public void renderPostShadows(List<VRef<VGImage>> vgOutImgs, Camera camera, ShaderStorageBuffer[] ssbos,
        ...
        // ⚡ Bolt: Removed Streams in hot loop to reduce per-frame object allocation and GC pressure
        int numOutImgs = vgOutImgs.size();
        int[] outImgsGlIds = new int[numOutImgs];
        int[] outImgsGlLayouts = new int[numOutImgs];
        for (int i = 0; i < numOutImgs; i++) {
        ...
        // ⚡ Bolt: Removed Stream mapping to prevent unnecessary List implementation allocation
        var outImgs = new ArrayList<VRef<VImage>>(numOutImgs);
        for (int i = 0; i < numOutImgs; i++) {
            outImgs.add(new VRef<>(vgOutImgs.get(i).get()));
        }
