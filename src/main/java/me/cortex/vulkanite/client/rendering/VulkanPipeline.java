...
public class VulkanPipeline {
    ...
    public void renderPostShadows(List<VRef<VGImage>> vgOutImgs, Camera camera, ShaderStorageBuffer[] ssbos,
        ...
        // Bolt optimization: Replace streams with pre-allocated arrays and loops
        // to prevent O(N) allocation and boxing overhead during this hot render path.
        var in = ctx.sync.createSharedBinarySemaphore();
        int[] outImgsGlIds = new int[vgOutImgs.size()];
        int[] outImgsGlLayouts = new int[vgOutImgs.size()];
        for (int i = 0; i < vgOutImgs.size(); i++) {
            outImgsGlIds[i] = vgOutImgs.get(i).get().glId;
        }
        Arrays.fill(outImgsGlLayouts, GL_LAYOUT_GENERAL_EXT);
        ...
        // Bolt optimization: Pre-allocate list and iterate directly to eliminate stream allocation
        List<VRef<VImage>> outImgs = new ArrayList<>(vgOutImgs.size());
        for (var vgImg : vgOutImgs) {
            outImgs.add(new VRef<>(vgImg.get()));
        }
