...
public class VulkanPipeline {
    ...
    public void renderPostShadows(List<VRef<VGImage>> vgOutImgs, Camera camera, ShaderStorageBuffer[] ssbos,
        ...
        // ⚡ Bolt: Removed .stream() allocations in hot path to reduce per-frame GC pressure
        int numImgs = vgOutImgs.size();
        int[] outImgsGlIds = new int[numImgs];
        int[] outImgsGlLayouts = new int[numImgs];
        ...
        // ⚡ Bolt: Replaced stream mapping with standard array list
        List<VRef<VImage>> outImgs = new ArrayList<>(numImgs);
        for (int i = 0; i < numImgs; i++) {
