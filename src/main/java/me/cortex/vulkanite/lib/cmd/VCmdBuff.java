...
public class VCmdBuff extends VObject {
    ...
    public void bindDSet(VRef<VDescriptorSet>... sets) {
        // ⚡ Bolt: Replaced Arrays.stream().mapToLong() to prevent iterator and lambda allocations.
        ...
        // ⚡ Bolt: Replaced Arrays.stream().map().toList() to prevent iterator and list allocations.
        for (int i = 0; i < sets.length; i++) {
    ...
    public void bindDSet(List<VRef<VDescriptorSet>> sets) {
        // ⚡ Bolt: Replaced stream().mapToLong() to prevent iterator and lambda allocations.
        ...
        // ⚡ Bolt: Replaced stream().map().toList() to prevent iterator and list allocations.
        for (int i = 0; i < numSets; i++) {
