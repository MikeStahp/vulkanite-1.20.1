    ...
    public VRef<VRaytracePipeline> build(VContext context, int maxDepth) {
        ...
        try (var stack = stackPush()) {
...

            {
                ...
                long[] layoutArray = new long[layouts.size()];
                int idx = 0;
                for (VRef<VDescriptorSetLayout> layout : layouts) {
                    layoutArray[idx++] = layout.get().layout;
                }
                layoutCreateInfo.pSetLayouts(stack.longs(layoutArray));
            }
...

            {
                ...
                return new VRef<>(new VRaytracePipeline(context, pPipeline.get(0), pLayout.get(0), sbtMapRef,
                        ...
                        VkStridedDeviceAddressRegionKHR.calloc().set(sbtMap.deviceAddress() + missGroupBase,
                                handleSizeAligned, handleSizeAligned * missGroupCount),
                        VkStridedDeviceAddressRegionKHR.calloc().set(sbtMap.deviceAddress() + hitGroupsBase,
                                handleSizeAligned, handleSizeAligned * hitGroupsCount),
                        VkStridedDeviceAddressRegionKHR.calloc().set(sbtMap.deviceAddress() + callGroupBase,
                                handleSizeAligned, handleSizeAligned * callGroupCount),
                        shaders,
                        reflection));
            }
        }
